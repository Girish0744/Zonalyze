...
def _center_for_municipality(municipality_name: str) -> Tuple[float, float]:
    return MUNICIPALITY_CENTERS.get(municipality_name, MUNICIPALITY_CENTERS["Kitchener"])
...
def build_geospatial_market_context(request: AnalyzeScenarioRequest) -> GeospatialMarketContext:
    ...
    return GeospatialMarketContext(
        ...
        coverage_note=(
            "The radius is centered on the selected municipality. Competitor, transit, and commercial activity markers use live OpenStreetMap coordinates when available."
        ),
        evidence_note=(
