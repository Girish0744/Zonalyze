...
def _catalog_record(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    name = config.get("business_subcategory") or config.get("name")
    if not name:
        raise ValueError(f"Business catalog entry missing business_subcategory/name: {config}")

    group = config.get("business_group") or config.get("category") or "General Local Business"
    ...
    return {
        ...
        "lease_sensitivity": _extract_float(config, ["lease_sensitivity", "lease_sensitivity_0_1"], 0.55),
        "competition_sensitivity": _extract_float(config, ["competition_sensitivity", "competition_sensitivity_0_1"], 0.55),
        "demand_sensitivity": _extract_float(config, ["demand_sensitivity", "demand_sensitivity_0_1"], 0.65),
        "target_customer_rate": _extract_float(config, ["target_customer_rate", "target_customer_rate_0_1"], 0.045),
        "operating_cost_multiplier": _extract_float(config, ["operating_cost_multiplier"], 1.0),
        "repeat_customer_factor": _extract_float(config, ["repeat_customer_factor"], 1.0),
    }
...
def _sample_row(profile: MunicipalityProfile, business: Dict[str, Any], rng: np.random.Generator) -> Dict[str, Any]:
    ...
    expected_customers_per_day = _bounded(
        (target_customer_pool / 30.0) * (0.35 + demand_pressure / 100.0) * (1.2 - competition_pressure / 220.0),
        1,
        2000,
    )
    ...
    risk_score = _bounded(
        ...
        + competition_pressure * 0.20
        + rent_pressure * 0.22
        - demand_pressure * 0.14
        + max(0, -monthly_net_revenue / 12000.0) * 20
        + rng.normal(0, 5),
        0,
        100,
    )
